...
namespace Testing.UnitTest
{
	...
	public class EqualAsserter : EqualityAsserter
	{
		...
		public override bool Test()
		{
			...
			if( expected.GetType().IsArray && actual.GetType().IsArray )
			{
				...
				else
				{
					...
					if( !ArrayRankEqual( expectedArray, actualArray, ref rank, indices ) )
					{
						DisplayArrayDifferences( rank, indices );
						areEqual = false;
					}
				}
			}
		...
		private void DisplayArrayDifferences( int rank, int[] indecies )
		{
			FailureMessage.DisplayArrayDifferences( (Array)expected, (Array)actual, rank, indecies );
		}
	}
}
